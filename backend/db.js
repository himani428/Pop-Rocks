...
// No native modules required, so it installs and runs anywhere (local, Render, Vercel-style hosts)
// with nothing more than `npm install`. For a small studio site this is more than enough;
// swap this file out for a real database (Postgres/Mongo) later without touching the routes much.
...
function ensureDb() {
  ...
  if (!fs.existsSync(DB_FILE)) {
    ...
    if (!current.classes || current.classes.length === 0) {
      current.classes = DEFAULT_CLASSES;
      changed = true;
    }
    if (!current.services || current.services.length === 0) {
      current.services = DEFAULT_SERVICES;
      changed = true;
    }
    if (!current.trainers || current.trainers.length === 0) {
      current.trainers = DEFAULT_TRAINERS;
      changed = true;
    }
    if (!current.pricing || !current.pricing.regular) {
      current.pricing = DEFAULT_PRICING;
      changed = true;
    }
    if (changed) writeDb(current);
  }
}
